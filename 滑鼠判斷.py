...
while run:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            run = False
        # elif event.type == pygame.MOUSEMOTION: 取得滑鼠位置一次
        #     print(event.pos)
        elif event.type == pygame.MOUSEBUTTONDOWN:#滑鼠判斷但只能判斷一次
            if event.button == 1:
                print("按了左鍵")
            elif event.button == 3:
                print("按了右鍵")
    # buttoms = pygame.mouse.get_pressed() #每秒判斷一次
    # if buttoms[0]:
    #     print("按住左鍵")
    # if buttoms[1]:
    #     print("按住中鍵")
    # print(pygame.mouse.get_pos()) #每秒取得滑鼠座標
    #更新
